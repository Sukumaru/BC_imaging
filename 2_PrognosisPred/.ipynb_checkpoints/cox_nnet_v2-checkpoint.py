...
############################ made to work for Python3 by Shashank Yadav ##################
...
class HiddenLayer(object):
    def __init__(self, rng, input, n_samples, map, label, activation=T.nnet.elu):
...
class CoxMlp(object):
    def __init__(self, x_train, rng, n_samples, node_map, input_split):
        ...
        if node_map == None:
            #self.node_map = [[(x_train.shape[1],[0],numpy.ceil(2*(x_train.shape[1] ** 0.5)))]]
            self.node_map = [[(x_train.shape[1],[0],numpy.ceil((x_train.shape[1] ** 0.5)))]]
            #self.node_map = [[(x_train.shape[1],[0],128)]]
        else:
        ...
        self.L2_sqr = 0
        for i in range(len(self.W)):
            self.L2_sqr = self.L2_sqr + T.sum(T.pow(self.W[i],2))  #L2 regression formula
            #self.L2_sqr = self.L2_sqr + T.sum(T.abs_(self.W[i])) # testing L1 regression just formula is changed; didn't work as expected
...
def defineSearchParams(search_params):
    ...
    return(method, learning_rate, momentum, beta1, beta2, epsilon, lr_decay, lr_growth, eval_step, max_iter, stop_threshold, patience, patience_incr, rand_seed)
...
def trainCoxMlp(x_train, ytime_train, ystatus_train, model_params = dict(), search_params = dict(), verbose=False):
    ...
    method, learning_rate, momentum, beta1, beta2, epsilon, lr_decay, lr_growth, eval_step, max_iter, stop_threshold, patience, patience_incr, rand_seed = defineSearchParams(search_params)
    ...
    cost = (model.negative_log_likelihood(train_ystatus)+ L2_reg * model.L2_sqr)
...
def crossValidate(x_train, ytime_train, ystatus_train, model_params = dict(),search_params = dict(),cv_params = dict(), verbose=False):
    ...
    #cv_folds=cross_validation.KFold(N_train,n_folds=n_folds, shuffle=True, random_state=cv_seed) #old python2 way of doing things
    cv_folds=model_selection.KFold(n_splits= 5, shuffle=True, random_state=cv_seed)
    k=0
...
def L2Profile(x_train, ytime_train, ystatus_train, x_validation, ytime_validation, ystatus_validation, model_params = dict(),search_params = dict(),cv_params = dict(), verbose=False):
    ...
    N_train = ytime_train.shape[0]
    likelihoods = []
    for i in tqdm(range(len(L2_range))):
        model_params['L2_reg'] = numpy.exp(L2_range[i])
...
def saveModel(model, file_name):
    ...
    dill.dump( (W,b, node_map, input_split, n_samples, x_train, rng), open( file_name, "wb" ))
...
def loadModel(file_name):
    ...
    for i in range(len(W_l)):
        model.W[i].set_value(W_l[i])

    for i in range(len(b_l)):
        model.b[i].set_value(b_l[i])
...
def imp_original(model_filename,hidden_imp):
    ...
    weights = list(W)
    weights = weights[0]
    print(weights.shape) # verify correct loading

    orig_importance = numpy.dot(weights,hidden_imp)
